...
## Исправления после сверки с полным текстом задания (не по пересказу)

При повторной сверке с точным текстом задания нашлись расхождения, все исправлены отдельным
коммитом на ветке `fix/lab7-lab8-spec-compliance` (влита в `main`):

- Установлен `react-router-dom` (задание явно просит поставить его сейчас, даже неиспользуемым
  в этой лабе — понадобится для будущей маршрутизации).
- Компоненты переразложены по `components/ui/`, `components/layout/`, `components/features/`
  вместо плоского `components/`; добавлена заготовка `src/pages/`.
- Добавлен базовый переиспользуемый UI-компонент `ui/Card.jsx` (используется в `RepoStatsCard`,
  где карточки — обычные `<div>`, без потери семантики; в `FeatureCard` семантический тег
  `<article>` сохранён, класс `.card` подключён только для общих визуальных стилей).
- Добавлен обработчик-заглушка: кнопка «Подробнее» на каждой `FeatureCard` вызывает
  `onLearnMore(title)`, реализованный в `App.jsx` как `console.log(...)` — без изменения
  состояния, ровно как требует задание.
- `features` в `mockData.js` расширен с 4 до 5 записей.
- Добавлен `frontend/README.md` с предметной областью, инструкцией запуска и списком компонентов.
